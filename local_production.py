...
MODELS_DIR = 'models'  # Directory to save models and vectorizers
os.makedirs(MODELS_DIR, exist_ok=True)

topk = 3  # top K predictions for MAP@3

# =========================================================
# GPU Detection (for completeness, but will run CPU)
# =========================================================
def has_gpu():
    try:
...
# Save vectorizers and label encoders for API use
joblib.dump(word_vect, os.path.join(MODELS_DIR, 'word_vect.pkl'))
joblib.dump(char_vect, os.path.join(MODELS_DIR, 'char_vect.pkl'))
joblib.dump(ques_vect, os.path.join(MODELS_DIR, 'ques_vect.pkl'))
joblib.dump(cat_le, os.path.join(MODELS_DIR, 'cat_le.pkl'))
joblib.dump(mis_le, os.path.join(MODELS_DIR, 'mis_le.pkl'))

# =========================================================
# LightGBM training with tuned parameters for CPU
# =========================================================
...
# =========================================================
# Oversample helper optimized for sparse data
# =========================================================
...
# =========================================================
# Hierarchical prediction with saving models
# =========================================================
def hierarchical_predict(train_df, test_df, topk=3):
    ...
    # Save the top-level "is_true" models for each fold
    is_true_models = []
    ...
    for fold, (tr_idx, val_idx) in enumerate(skf.split(train_df, train_df['is_true'])):
        ...
        is_true_models.append(model)
        # Save top-level model per fold
        joblib.dump(model, os.path.join(MODELS_DIR, f'is_true_model_fold{fold}.pkl'))
    ...
    branch_models = {}  # To save all branch models
    ...
    for branch in [0, 1]:
        ...
        for fold, (tr_idx, val_idx) in enumerate(StratifiedKFold(n_splits=4, shuffle=True, random_state=42).split(branch_df, branch_y)):
            ...
            # Save branch model for each fold
            model_filename = os.path.join(MODELS_DIR, f'branch_{branch}_model_fold{fold}.pkl')
            joblib.dump(model, model_filename)
            branch_models[branch].append(model)
...
# =========================================================
# Misconception prediction with saving models
# =========================================================
def misconception_predict(train_df, test_df, topk=3):
    ...
    for fold, (tr_idx, val_idx) in enumerate(skf.split(train_df, train_df['Misconception_enc'])):
        tr, val = train_df.iloc[tr_idx], train_df.iloc[val_idx]
        X_tr = X_all[tr_idx]
        X_val = X_all[val_idx]
        X_test = X_test_all
        ...
        mis_models.append(model)
        # Save misconception model per fold
        joblib.dump(model, os.path.join(MODELS_DIR, f'misconception_model_fold{fold}.pkl'))
...
# =========================================================
# Combined MAP@3 for Category:Misconception
# =========================================================
def combined_map3(train_df, category_oof_probs, mis_oof_probs, topk=3):
    # Create combined label space
    ...
    # Generate combined prediction probabilities
    ...
    print(f"Combined Category:Misconception MAP@3: {map3_score:.4f}")
